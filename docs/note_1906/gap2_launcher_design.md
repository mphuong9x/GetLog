...
> **Tiến độ (2026-06-19):** L-0 → L-2 ✅; L-3 **slice `status`** ✅; **L-4 `run/stop/restart`** ✅; **L-5 auto-start launcher** ✅; **L-6 WPF launcher MVP** ✅ DONE; **L-7 đóng gói + verify wire** ✅ DONE (agent test 83/83). **L-3 slice 2 ✅ ĐÓNG**: 2a `deploy`/gate-Run + 2b `justUpdatedTo` + 2c `recentEvents` DONE (agent test 86/86, launcher build OK); 2d `iconBase64` chủ động bỏ (tránh dep cosmetic). **GAP-2 chỉ còn duy nhất: L-7 nghiệm thu thủ công trên 1 trạm thật có người vận hành** (GUI/reboot/đổi ca — cần desktop, không tự động hoá được). Chi tiết: xem cuối §9.
...
4. **L-3** `IpcServer` (named pipe + `status`) — **slice `status` ✅ DONE**: `IpcServer : BackgroundService` (named pipe + ACL `BuiltinUsers`+`LocalSystem`, JSON-line, op `status`/unknown), `AgentStatusProvider` (map catalog+supervisor+token→`StatusResponse`), `ServerReachabilitySignal` (time-based, set ở heartbeat OK), DI + `IpcProtocol.PipeName` (shared contracts). Test `AgentStatusProviderTests` + `IpcServerTests` (round-trip pipe thật). **slice 2a `deploy` per-package ✅ DONE (2026-06-19, agent test 84/84):** `DeployProgressRegistry` (in-memory singleton, mirror `JobCancellationRegistry`); `JobExecutor` Set khi Downloading/Installing (mẫu số = `DeltaSize`) + Clear ở `finally` (mọi nhánh); `AgentStatusProvider` map `Deploy` + ép `Status="Updating"` (thắng Idle/Running). Launcher L-6 đã gate Run sẵn trên `Deploy!=null` → gate **hết trơ**, hoạt động end-to-end. **slice 2b `justUpdatedTo` ✅ DONE (agent test 85/85):** suy từ `CatalogApp.DeployedAt` qua cửa sổ `JustUpdatedWindow=1 phút` (KHÔNG thêm state, KHÔNG đụng JobExecutor — lệch nhẹ spec "1 nhịp" để badge thấy được + bền qua agent-restart), suppress khi đang deploy; launcher hiện badge "MỚI CẬP NHẬT" (mirror badge ACTIVE, build net48 OK). **slice 2c `recentEvents` ✅ DONE (agent test 86/86):** `RecentEventsBuffer` (in-mem singleton bounded 12, mirror `JobCancellationRegistry`); `ProcessSupervisor` record `Started` (StartProcessAsync) + `Stopped`/`Crashed` (WatchAsync theo `ExitReason`, skip HostShuttingDown) qua param optional `=null` (không phá 4 test ctor); `JobExecutor` record `Updated` sau upsert; `AgentStatusProvider` resolve tên từ catalog; launcher có pane "Sự kiện gần đây". **slice 2d `iconBase64` — CHỦ ĐỘNG BỎ QUA (2026-06-19):** cần thêm dep `System.Drawing.Common` vào agent production cho tính năng thuần thẩm mỹ; launcher đã có đèn trạng thái màu → không xứng chi phí. `IconBase64` để null. ⇒ **slice 2 đóng (chỉ còn 2d nếu sau này cần icon).**
...
8. **L-7** **Phân phối + đóng gói** — **✅ DONE (phần tự động hoá được)**:
   - **Đóng gói**: `scripts/package-agent.ps1` (dotnet publish agent net8 self-contained single-file + VS MSBuild launcher net48 → bundle `MProjectLauncher.exe` + `Newtonsoft.Json.dll` + contracts.dll **cạnh** `MProjectAgent.exe` trong `artifacts/MProjectAgent-win-x64/`). `LauncherBootstrapper` tự tìm launcher ở `AppContext.BaseDirectory` → **không phải sửa `ServiceInstaller`** (chỉ `sc create` agent exe; cài bằng `MProjectAgent.exe install`). Đã chạy ra package thật OK.
   - **Verify wire (rủi ro #1)**: `IpcWireCompatTests` round-trip DTO thật qua **đúng 2 serializer production** (STJ ↔ Newtonsoft 13.0.1) — camelCase, `DateTimeOffset` precision, nullable, nested, list, null-omit `packageId` đều khớp. + **smoke test cross-process thật**: agent publish boot → phục vụ pipe → `status` trả camelCase JSON đúng, op lạ trả `ok:false`.
   - **CÒN LẠI (chỉ nghiệm thu thủ công trên trạm thật có người):** 1-app auto-run, 2-app chọn/đổi, mất mạng, đổi ca, reboot giữ active, launcher GUI render + bung-tự-động. Kịch bản "đang-update gate Run" cần **L-3 slice 2** (field `deploy`) mới test trọn.
