...
> **`F-02` Resolved (2026-06-10):** Thêm `Station.UpdateWindow` (jsonb, nullable, value object `StationUpdateWindow { DailyStartUtc, DailyEndUtc, DaysOfWeek }` — migration `AddStationUpdateWindow`). Giờ lưu **UTC**; `null` = không giới hạn (mặc định, giữ hành vi cũ). `PollAsync` dùng chung `CanEnqueueNewJobsAsync` để filter: ngoài window → không tạo job Pending mới (job active sẵn có vẫn tiếp tục). `UpdateStationRequest`/`StationDto` thêm field `UpdateWindow` để admin set qua `PUT /api/v{version}/stations/{id}`. Test mới: `Poll_SkipsNewJobEnqueue_OutsideStationUpdateWindow`, `Poll_CreatesJob_InsideStationUpdateWindow`. **FE follow-up Resolved (2026-06-11):** thêm `StationEditModal` ([ProductModals.tsx](../MProjectFrontend/src/pages/Product/components/ProductModals.tsx)) với toggle maintenance window + `TimePicker.RangePicker` (UTC, `order={false}` cho phép wrap qua nửa đêm) + `Checkbox.Group` chọn ngày (trống = mọi ngày). Trang Product giờ có nút Edit per station + cột "Maintenance window" hiển thị tóm tắt; API `stationsApi.updateStation` → `PUT /api/stations/{id}`. Times treat thuần component giờ/phút (không convert timezone) nên số admin nhập = số lưu UTC.
...
> **`F-03` Resolved (2026-06-11):** Server-pushed commands qua heartbeat. **Backend:** entity `AgentCommand` (`Id`, `ComputerId`, `Type`, `PayloadJson?`, `EnqueuedAt`, `EnqueuedBy?`, `DeliveredAt?`, `AckedAt?`, `AckResult?`); enum `AgentCommandType { CancelJob, Restart, KillAndRestart, ReloadConfig, ForceMaintenanceOff }`; migration `AddAgentCommandQueue` + index `IX_AgentCommands_Computer_Pending(ComputerId, AckedAt)`. Service `AgentCommandService` với `EnqueueAsync` / `ClaimPendingForDeliveryAsync` (stamp `DeliveredAt`, take ≤ 16/heartbeat theo FIFO `EnqueuedAt`) / `AckAsync` (idempotent, check `agent.ComputerId == command.ComputerId`). `AgentHeartbeatResponse.Commands` mở rộng; `RecordHeartbeatAsync` claim + trả về commands sau khi heartbeat persist xong. Endpoint mới `POST /agent/v1/commands/{id}/ack` ở [AgentController.cs:194](MProjectBackend/MProject.Api/Controllers/Assets/AgentController.cs:194). `InstallationJobService.CancelJobAsync` hook: nếu cancel job đang `Downloading|Installing` → enqueue `CancelJob { JobId }` (Pending không enqueue vì agent chưa kéo manifest). **Agent:** DTO mirror, `ServerClient.AckCommandAsync`, `JobCancellationRegistry` (concurrent set), `AgentCommandHandler` dispatch: `CancelJob` → mark registry để `JobExecutor` bỏ ở 3 checkpoint (trước ack, sau resolve, sau download); `Restart` → `ProcessSupervisor.RequestRestartAsync(gracefulFirst: true)` (CloseMainWindow + 5s grace rồi Kill); `KillAndRestart` → kill ngay; `ForceMaintenanceOff` → xoá maintenance flag file; `ReloadConfig` → no-op v1 + log. Mỗi command ack sau xử lý, ack thất bại sẽ được redeliver heartbeat sau. **Tests:** `AgentCommandServiceTests` (6 case: enqueue / unknown computer / claim stamps delivered / ack idempotent / ack cross-agent rejected / heartbeat trả commands) + `InstallationJobServiceTests.CancelJob_DuringDownloading_EnqueuesAgentCancelCommand` + `CancelJob_WhilePending_DoesNotEnqueueAgentCommand`. **Follow-up:** admin REST endpoint enqueue thủ công (`POST /api/computers/{id}/commands`) chưa làm — chờ FE need; CancelJob không gửi ack body terminal về server vì job đã ở terminal status DB sau `CancelJobAsync`.

> **`F-04` Resolved (2026-06-11):** Inventory + drift detection. **Backend:** `PcInstallationRecord` thêm `LastInventoryAt`, `DriftStatus` (enum `PcInstallationDriftStatus: Unknown/Healthy/Drift`), `DriftSummary` (nvarchar 2048); migration `AddPcInstallationDriftFields` + index trên `DriftStatus`. DTOs `AgentInventoryRequest`/`AgentInventoryPackage`/`AgentInventoryFileIssue` thay thế placeholder cũ. `PcInventoryService.ProcessAsync(agentId, request)` resolve agent → computer, lookup records theo `(ComputerId, VersionId IN reported)`, set `DriftStatus = Healthy` nếu `Issues.Count == 0` hoặc `Drift` ngược lại, build summary "N issue(s) of M files (...): paths". Unknown version packages (record không tồn tại server-side) log info + đếm `UnknownPackages`. Endpoint `POST /agent/v1/inventory` ở [AgentController.cs:170](MProjectBackend/MProject.Api/Controllers/Assets/AgentController.cs:170) (đổi từ 501 sang wire-up service). Admin `GET /api/v{version}/computers/drift` (`[RequirePermission(ManageComputers)]`) trả paged `ComputerDriftDto` aggregate theo computer. **Agent:** `CacheIndex.GetAllDeployedFilesAsync` (mới); `InventoryReporter.BuildReportAsync` group `DeployedFiles` theo `(PackageId, VersionId, InstallRoot)`, mỗi file check `File.Exists` → "missing", hash mismatch → "hash_mismatch", IO/Unauthorized → "unreadable". `ServerClient.ReportInventoryAsync` POST request. `AgentWorker.RunInventoryLoopAsync` chạy song song heartbeat/poll: dùng marker file `inventory.last` để skip nếu chạy gần đây, default `Agent:InventoryIntervalHours = 24` (0 disable), startup delay 2 phút (hoặc remainder của interval). **Tests:** 5 backend (`PcInventoryServiceTests`) + 5 agent (`InventoryReporterTests`). **FE follow-up Resolved (2026-06-11):** thêm tab "Drift" thứ ba trong [Computers.tsx](../MProjectFrontend/src/pages/Computer/Computers.tsx) (cùng permission `ManageComputers` với endpoint) — `DriftTable` ([components/DriftTable.tsx](../MProjectFrontend/src/pages/Computer/components/DriftTable.tsx)) liệt kê PC drift, expandable row hiển thị từng package lỗi + `driftSummary`; `computersApi.getDriftedComputers` → `GET /api/computers/drift`; summary card "Drifted" + badge đếm trên tab. Không cần route/menu mới.

> **`F-06` Resolved (2026-06-10):** Opt-in health probe per `SoftwareVersion.HealthCheckUrl` (migration `AddSoftwareVersionHealthCheckUrl`). Server validate URL ở [SoftwareVersionService.NormalizeHealthCheckUrl](MProjectBackend/MProject.Application/Services/Software/SoftwareVersionService.cs:115) (chỉ chấp nhận http/https absolute), expose qua `SetSoftwareVersionEntryPointRequest.HealthCheckUrl` và `AgentManifestJobResponse.HealthCheckUrl`. Agent thread URL qua `LaunchRequest`/`SupervisedProcessState`; `ProcessSupervisor` start probe loop (interval 30s, timeout 5s, threshold 3 — config qua `SupervisorOptions`). Sau threshold consecutive failures → kill process với `ExitReason.HealthCheckFailed` (preserve qua `_pendingKillReason` để không bị `ExitClassifier.Classify` overwrite); existing `ShouldRestart` policy handle restart bình thường. Probe stop khi process exit, supervisor stop, hoặc maintenance. Tests: 3 case validate URL ở backend. **FE follow-up Resolved (2026-06-11):** thêm ô input `Health check URL` trong `EntryPointPanel` ([ManifestModal.tsx](../MProjectFrontend/src/pages/Software/components/ManifestModal.tsx)) — `setVersionEntryPoint` giờ gửi kèm `healthCheckUrl`; manifest response (`SoftwareVersionManifestResponse`) thêm field `HealthCheckUrl` để pre-fill giá trị hiện tại (backend `SoftwareVersionService.GetManifestAsync` map thêm). Bỏ trống = tắt probe. Tiện thể fix lỗi TS pre-existing ở [ComputersTable.tsx:68](../MProjectFrontend/src/pages/Computer/components/ComputersTable.tsx#L68) (optional param đứng trước required param chặn `tsc -b`).

> **`S3-XX` Resolved (2026-06-10):** Gom S3 cleanup batch.
> - `S3-01`: trim `(M4, done)` tag khỏi comment `TryLaunchWithSupervisorAsync` ([JobExecutor.cs:185](MProjectAgent/Services/JobExecutor.cs:185)). Phần còn lại của comment block đã được xoá vì design giờ self-evident.
> - `S3-02`: **moot** — `docs/agent_note.md` đã được xoá khỏi repo ở revision trước (git status D), không cần cập nhật.
> - `S3-03`, `S3-04`, `S3-05`, `S3-06`, `S3-11`: **đã làm từ trước** (Win32ErrorCodes const, supervisor watch lưu task ref + await trong DisposeAsync, BlobCache PutAsync inline hash, MaintenanceCommand --path validation, EnrollCommand --token cho leading dash). Audit confirmed.
> - `S3-07`: audit confirmed — controller dùng `[RequirePermission(AppPermissions.ManageComputers)]` không pass `resourceIdRouteKey` → filter gọi `CheckPermissionAsync(..., null)` → check GLOBAL ManageComputers. Đúng level cho unassigned PCs.
> - `S3-08`: audit confirmed — cache key `|` separator vẫn an toàn (GUIDs, hex version tokens, dot-permission codes không chứa `|`); design comment ở [AuthorizationService.cs:148](MProjectBackend/MProject.Application/Services/Identity/AuthorizationService.cs:148) đã document invariant. Không thay đổi code.
> - `S3-09`: narrow `DbUpdateException` catch trong `SoftwareVersionService.CreateVersionAsync` thành `IsVersionNumberConflict` (PG UniqueViolation hoặc InMemory SoftwareVersion entry) — consistent với S2-12 pattern. **Không** chuyển sang DB sequence vì date-prefix design (`dd.MM.yyyy_NN`, reset hàng ngày) không hợp với global sequence; retry-on-unique-violation kết hợp composite unique index đã đúng pattern.
> - `S3-10`: audit phát hiện coverage bug ở `ComputerStatusMapper.DeriveLegacy` — `CrashLoop` đang fallthrough sang Online/Offline, che mất trạng thái lỗi. Rewrite thành exhaustive switch với `_ => throw`, thêm `CrashLoop → ComputerStatus.Error`. Đồng bộ projection ở [ComputerService.ProjectToDto](MProjectBackend/MProject.Application/Services/Assets/ComputerService.cs:249). Test matrix cập nhật phủ Running/Maintenance/CrashLoop + case throw cho unknown enum.
...
- [x] `F-03` Server-pushed commands (backend + agent; admin enqueue REST endpoint follow-up)
- [x] `F-04` Inventory + drift detection (backend + agent + FE types pending — `ComputerDriftDto` API ready)
- [ ] `F-05` Rollback automatic
- [x] `F-06` Test app health probe (backend + agent + FE types; FE admin form input để nhập `HealthCheckUrl` còn thiếu, follow-up riêng)
- [x] `S3-XX` All S3 cleanup (1 PR gom)
...
Tóm tắt tác dụng của fix F-03
Vấn đề trước fix: server không có kênh push xuống agent. Khi admin/server cần can thiệp một PC đang chạy (cancel job đang Download/Install dài 10 phút, restart test app, tắt cờ maintenance dính lại...), agent chỉ phát hiện qua poll tiếp theo hoặc timeout — chậm, lãng phí băng thông, và tạo race với S1-02 (CompleteAsync idempotent chỉ chịu được callback "completed" muộn, không khử được việc agent vẫn cố deploy bản đã cancel).

Sau fix, các tác dụng cụ thể:

Đóng race cancel-during-download. Admin bấm Cancel khi job đang Downloading → backend enqueue CancelJob { JobId } đồng thời với DB cancel → heartbeat kế tiếp (≤ interval mặc định 15s) agent nhận command, registry mark cancelled, JobExecutor skip ở 3 checkpoint (trước ack, sau resolve, sau download). Tiết kiệm băng thông và chắc chắn không deploy/launch version đã hủy.
Admin remote-control PC qua dashboard (chuẩn bị, không cần SSH). Có sẵn 4 lệnh: Restart (CloseMainWindow + 5s grace → Kill), KillAndRestart (kill ngay), ForceMaintenanceOff (xoá flag file mà operator quên), ReloadConfig (placeholder no-op). Tận dụng ProcessSupervisor đã có, không phát sinh API mới phía agent.
Mở đường cho F-05 auto-rollback. Plan chỉ rõ F-05 là prerequisite F-03; giờ command bus xong, F-05 chỉ cần enqueue KillAndRestart + tạo job rollback ở server-side là đủ.
Đảm bảo at-least-once + idempotent. Command có DeliveredAt (stamp khi heartbeat lấy) tách khỏi AckedAt (stamp khi agent xử lý xong). Ack thất bại → redeliver heartbeat sau; ack lần 2 không ghi đè ack lần 1 (if (command.AckedAt != null) return). AckAsync validate agent.ComputerId == command.ComputerId để chặn cross-machine ack.
Audit trail tối thiểu, sẵn sàng cho F-09. EnqueuedBy/At, DeliveredAt, AckedAt, AckResult (512 char) đủ để dashboard hiển thị "Lệnh X gửi bởi user Y lúc Z, agent ack lúc T, kết quả ...".
Không over-engineer. 1 bảng + 1 enum 5 giá trị + 1 endpoint ack. Chưa thêm SchemaVersion, retry/TTL, broker, hay admin enqueue endpoint — khách hàng đầu tiên là CancelJobAsync gọi in-code; admin REST endpoint sẽ là task riêng khi FE thực sự cần.
Kết quả kỹ thuật: backend build sạch, agent build sạch, 367/367 test pass (gồm 6 test mới cho AgentCommandServiceTests + 2 test mới CancelJob_* trong InstallationJobServiceTests). Migration 20260610223302_AddAgentCommandQueue auto-generated bằng dotnet ef migrations add. Không có comment trong code thực thi.