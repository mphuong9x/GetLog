...
> **Tiến độ (2026-06-19):** L-0 → L-2 ✅; L-3 **slice `status`** ✅; **L-4 `run/stop/restart`** ✅; **L-5 auto-start launcher** ✅; **L-6 WPF launcher MVP** ✅ DONE (agent test 80/80; launcher net48 build OK qua VS MSBuild); **L-7 packaging + wire-E2E** ✅ DONE (`scripts/package-agent.ps1` bundle launcher cạnh agent; `scripts/e2e-wire-check.ps1` xác nhận seam Newtonsoft↔STJ — zero code change). Còn: L-3 slice 2 (deploy-progress + recentEvents + icon/justUpdated) + **L-7 thử trên trạm thật** (cần desktop + enroll). Chi tiết: xem cuối §9.
...
8. **L-7** **Phân phối + đóng gói** (xem §11) — **packaging + wire-E2E ✅ DONE; thử trạm thật còn lại**: `scripts/package-agent.ps1` build cả 2 đầu (agent `dotnet publish` self-contained single-file win-x64 + launcher net48 qua VS MSBuild tự dò bằng `vswhere`) rồi **bundle launcher cạnh agent** (`MProjectLauncher.exe` + `.exe.config` + `MProjectAgent.Ipc.Contracts.dll` + `Newtonsoft.Json.dll`) trong `artifacts\MProjectAgent-win-x64\`. KHÔNG sửa code: `LauncherBootstrapper` đã tự tìm `MProjectLauncher.exe` cạnh agent; `ServiceInstaller` không đổi (bundle xảy ra lúc đóng gói, không phải lúc install). **Verify E2E seam JSON (Newtonsoft↔STJ) ✅**: `scripts/e2e-wire-check.ps1` chạy agent đã publish, gửi `{"op":"status"}` qua named pipe, deserialize wire STJ bằng **DLL Newtonsoft + contracts thật đã bundle** (PowerShell 5.1 = .NET FW, đúng path `IpcClient`) → `OpResult.Ok/Status.Machine.Hostname/Apps` round-trip OK. **CÒN LẠI:** thử trên 1 trạm thật (1-app auto-run, 2-app chọn/đổi, đang-update gate Run, mất mạng, đổi ca, reboot giữ active) — cần desktop + agent enroll thật.
