...
- **Factory TLS trust (current open risk):** packaged Agent configuration enables
  `AllowUntrustedCertificate`, and the client wires it to accept any server
  certificate. `install-server.ps1` creates an SNI binding for host `te` on
  8443, while agent instructions use a LAN IP URL. Fix the hostname/SAN/binding
  contract and disable the bypass; accepting an untrusted certificate is not a
  deployment solution.
- **Manifest path containment (current open risk):** Agent deployment combines
  server-provided relative paths with the install root without canonicalizing
  the result. Package names are also used to derive folders. Validate on both
  server and Agent, use an immutable package ID for the folder, and reject
  rooted paths, traversal, reserved names, collisions, ADS, and reparse escapes.
...
- **Software approval boundary (current open risk):** Viewer is globally seeded
  with software create/publish/assign permissions; package approval policy is
  not exposed by the public package DTOs; and the assignment `activate` endpoint
  can enable a pending assignment without proving approval. Seeder changes only
  add grants, so revocation requires an explicit database reconciliation.
- **Frontend approval contract (current open risk):** Deployment Matrix treats a
  pending assignment like an inactive assignment and still exposes Activate;
  route/action permission gates do not match backend software permissions. The
  approval drawer omits target-specific change details, and approval lists page
  a locally sliced snapshot capped at 100. Return explicit approval state and
  immutable target snapshots, gate every mutation, and use server pagination.
- **Role/auth lifecycle (current open risk):** role-permission mutation does not
  reuse the grantability/no-self-elevation checks used by scoped authorization
  mutations. Disabling a user, resetting a password, or logging out revokes
